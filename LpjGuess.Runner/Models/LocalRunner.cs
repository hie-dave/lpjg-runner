...
public class LocalRunner : IRunner
{
    ...
    /// </summary>
    /// <param name="config">Configuration parameters controlling how the job should be run.</param>
    public LocalRunner(GlobalConfig config)
    {
        if (Environment.ProcessorCount > 64 && config.CpuCount > 64)
            throw new NotImplementedException("TODO: use platform-specific API to suppost >64 CPUs");
        if (config.CpuCount > Environment.ProcessorCount)
            throw new NotImplementedException($"cpu_count must be < NCPUs ({Environment.ProcessorCount} in this case), but is: {config.CpuCount}");
        this.config = config;
    ...
    public async Task RunAsync(Job job, CancellationToken ct)
    {
        ...
        proc.StartInfo.FileName = config.GuessPath;
        proc.StartInfo.ArgumentList.Add("-input");
        proc.StartInfo.ArgumentList.Add(config.InputModule);
